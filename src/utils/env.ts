...
// Using browser cache as a placeholder for now,
// will need to update if we want to use entirely in the browser
...
export interface TransformersEnvironment {
	...
	/**
	 * Whether to allow loading of local files, defaults to `false` if running in-browser,
	 * and `true` otherwise. If set to `false`, it will skip the local file check
	 * and try to load the model from the remote host.
	 */
...
//  * @property {{onnx: Partial<import('onnxruntime-web').Env>}} backends Expose environment variables of different backends,
//  * allowing users to set these variables if they want to.
